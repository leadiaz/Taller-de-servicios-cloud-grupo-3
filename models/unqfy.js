...
const UNQfy = /** @class */ (function () {
  ...
  UNQfy.prototype.existTrackInAlbum = function (atrack) {
    const bool = this.albums.some((album) => { return album.tracks.some((track) => { return track.name === atrack; }); });
    if (bool) {
  ...
  UNQfy.prototype.searchByName = function (nombre) {
    ...
    this.artists.forEach((artist) => { if (artist.name.includes(nombre)) {
      result.addArtist(artist);
    } });
    this.albums.forEach((album) => { if (album.name.includes(nombre)) {
      result.addAbum(album);
    } });
    this.tracks.forEach((track) => { if (track.name.includes(nombre)) {
  ...
  UNQfy.prototype.getTracksMatchingGenres = function (genres) {
    return this.tracks.filter((track) => { return track.anyGenre(genres); });
  };
  //retorna: los tracks de un genero en particular
  UNQfy.prototype.getTracksMatchingGenre = function (genre) {
    return this.tracks.filter((track) => { return track.genres.includes(genre); });
  };
